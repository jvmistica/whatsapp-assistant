...
def sms_reply():
    ...
    if "what can i make?" in action:
        possibilities = ", ".join(search_for_recipes())
        resp.message(f"You can make the following: {possibilities}")
        return str(resp)
    elif action[:3] == "new":
        new_record(text, action)
    elif action[:4] == "show":
        txt = view_record(text, action)
        resp.message(txt)
        return str(resp)
    elif action[:4] == "edit":
        edit_record(text, action)
    elif action[:6] == "delete":
        remove_record(text, action)
...
def view_record(text, action):
    ...
    if table_name == "Notes":
        ...
        result = show_details(text, action)
    return result


def remove_record(text, action):
...
def search_for_recipes():
    ...
    for text in recipes:
        ...
        needed_items = [i.replace("and ", "").replace(".", "").capitalize() \
                        for i in recipe_details[2].split(", ")]
